  ...
  for (var i = 0; i < height; i++) {
	...
	for (var y = 0; y < height; y++) {
		for (var x = 0; x < width; x++) {
			...
			// if ( x == 20 ) { console.log(vicinVals); }
			// if ( x == 20 ) { console.log(mat[y][x]); }
			simp[y][x] = Number(_.chain(vicinVals).countBy().pairs().max(_.last).head().value());
			// if ( x == 20 ) { console.log(simp[y][x]); }
		}
	}
	return simp;
};
